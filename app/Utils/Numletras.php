...
class Numletras
{
    function getTotalLetras($total, $moneda)
    {
        $totalVenta = explode(".",  number_format($total, 2));
        $totalLetras = $this->num2letras($totalVenta[0]);
        if (isset($totalVenta[1])) {
            return $totalLetras = 'Son: ' . $totalLetras . ' con ' . $totalVenta[1] . '/100' . " " . $moneda;
        } else {
            return $totalLetras = 'Son: ' . $totalLetras . ' con ' . '00/100' . " " . $moneda;
        }
    }
